...
public class AjaxSearch extends HttpServlet {
	...
	protected void actionDo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// System.out.println("AjaxSearch 서블릿의 actionDo() 메소드");
		request.setCharacterEncoding("UTF-8");
		...
		// write() 메소드의 인수로 반드시 문자열이 나와야 하고 ajax로 서블릿을 호출한 곳에서는
	...
	private String getJSON(String name) {
		...
		// 검색할 이름을 입력하기 않고 검색 버튼을 클릭했을 때 넘어오는 null을 공백으로 처리한다.
		if(name == null) {
			name = "";
		}
		...
		result.append("{\"result\": [");// json 시작 (json의 키는 ""를 사용해야 한다.)
		
		//데이터의 개수만큼 반복하면 json 형태의 문자열을 만든다.
		for(AjaxVO vo : list) {
