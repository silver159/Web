...
const searchRequest = new XMLHttpRequest(); // 검색용으로 사용할 XHR 객체
const insertRequest = new XMLHttpRequest(); // 입력용으로 사용할 XHR 객체

// ajax 검색 요청 함수
function searchFunction() {
	// console.log($('#username').val())
	
	// XHR 객체.open('요청방식','요청주소',동기방식),동기방식: 동기식(false), 비동기식(true)  
	// encodeURIComponent(): 문자열을 유니코드(UTF-8)로 인코딩 한다.
	
	// GET 방식 요청. 대부분 get 방식 사용
	let url = './AjaxSearch?name='+ encodeURIComponent($('#username').val());
	// console.log(url)
	// './' 무조건 시작
	searchRequest.open('GET', url, true);
	// send() 함수로 서버에 요청(서블릿 호출)한다.
	// 데이터를 url의 일부인 쿼리 스트링(?~~)으로 전송할 경우 send() 함수에 인수로 null을 
	// 지정한다.
	searchRequest.send(null);
	
	// POST 방식 요청. 실습용
	/*
	let url = './AjaxSearch'; // POST 방식은 url에 퀴리 스트링을 사용하지 않는다.
	searchRequest.open('POST', url, true);
	// GET 방식은 정보가 url에 담겨서 넘어가지만 POST 방식은 정보가 헤더에 담겨서 넘어간다.
	// setRequestHeaders() 함수로 반드시 헤더 정보를 포함시켜야 한다. 안바뀜
	searchRequest.setRequestHeader('content-Type', 'application/x-www-form-urlencoded');
	// 데이터를 send() 함수의 인수로 전달한다.
	searchRequest.send('name='+ encodeURIComponent($('#username').val()));
	*/
	
	// onreadystatechange를 사용해서 ajax 요청이 완료되면 자동으로 실행할 콜백 함수 이름을 
	// 지정한다. 
	searchRequest.onreadystatechange = searchProcess; // ()를 쓰면 안된다. 자동 실행된다.
}
...
// 통신이 정상적으로 완료되었음을 확인하고 필요한 작업을 실행한다.
	if (searchRequest.readyState == 4 && searchRequest.status == 200){
		...
		// 서블릿에서 리턴된 문자열을 javascript 객체로 변환하기 위해 괄호를 붙여서 eval()
		// 함수를 실행해서 객체로 저장한다.
		let object = eval('('+ searchRequest.responseText +')')
		// console.log(object); // result:Array(1)
		// javascript 객체에서 result라는 key에 할당된 데이터를 얻어온다. 
		// => 화면에 출력할 데이터
		let result = object.result;
		// console.log(result); // 0 : Array(5)
...
function insertFunction() {
	...
	let url = './AjaxInsert?name='+ encodeURIComponent($('#name').val())
		...
		+'&gender='+ encodeURIComponent($('input[name=gender]:checked').val()) // 라디오 작업
	...
	insertRequest.onreadystatechange = inertProcess;
}
