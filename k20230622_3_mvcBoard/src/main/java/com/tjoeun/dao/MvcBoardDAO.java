...
// MvcBoardService 클래스에서 호출되는 mapper와 테이블에 저장할 메인글 정보가 저장된 객체를
// 넘겨받고 메인글을 테이블에 저장하는 mvcboard.xml 파일의 insert sql 명령을 실행하는 메소드
	public void insert(SqlSession mapper, MvcBoardVO vo) {
		System.out.println("MvcBoardDAO 클래스의 insert() 메소드");
		mapper.insert("insert", vo);
	}
	
// MvcBoardService 클래스에서 호출되는 mapper를 넘겨받고 테이블에 저장된 전체 글의 개수를 
...
// HashMap 객체를 넘겨받고 1페이지 분량의 글 목록을 얻어오는 mvcboard.xml 파일의 select sql 
// 명령을 실행하는 메소드
...
// MvcBoardService 클래스에서 호출되는 mapper와 조회수를 증가시킬 글번호를 넘겨받고 조회수를 
// 증가시키는 mvcboard.xml 파일의 update sql 명령을 실행하는 메소드	
...
// MvcBoardService 클래스에서 호출되는 mapper와 조회수를 증가시킨 글번호를 넘겨받고 조회수를 
// 증가시킨 글 1건을 얻어오는 mvcboard.xml 파일의 select sql 명령을 실행하는 메소드		
...
// MvcBoardService 클래스에서 호출되는 mapper와 gup와 seq가 저장된 hmap 객체를 넘겨받고 
// 조건에 만족하는 글의 seq를 1씩 증가시키는 mvcboard.xml 파일의 update sql 명령을 실행하는 
// 메소드	
