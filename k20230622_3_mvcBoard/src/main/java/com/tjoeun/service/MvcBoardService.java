...
// MvcBoardDAO 클래스에서 사용할 ㅇ매 클래스 객체를 얻어온다.	
	private MvcBoardDAO dao = MvcBoardDAO.getInstance(); // 많이 써서 필드로 생성	
	
// 컨트롤러에 insertOK.nhn 이라는 요청이 들어오면 컨트롤러에서 호출하는 메서드로 테이블에 저장할
// 메인글 정보가 저장된 request 객체를 넘겨받고 mapper를 얻어온 후 MvcBoardDAO 클래스의 insert
// sql 명령을 실행하는 메소드를 호출하는 메소드
	public void insert(HttpServletRequest request, HttpServletResponse response) {
		System.out.println("MvcBoardService 클래스의 insert() 메소드");
		SqlSession mapper = MySession.getSession();
		
		// insert.jsp에서 입력한 request 객체에 저장되서 넘어오는 데이터를 받아서 MvcBoardVO 
		// 클래스 객체에 저장한다.
		...
		// MvcBoardDAO 클래스의 insert.jsp에서 입력한 데이터를 테이블에 저장하는 insert sql
		// 명령을 실행하는 메소드를 호출한다.
...
// 컨트롤러에 list.nhn 이라는 요청이 들어오면 컨트롤러에서 호출하는 메서드로  mapper를 얻어온 
// 후 MvcBoardDAO 클래스의 브라우저에 출력할 1페이지 분량의 글 목록과 페이징 작업에 사용할
// 8개의 변수가 저장된 클래스 객체를 만들어 request 영역에 저장하는 메소드를 호출하는 메소드	
...
// 컨트롤러에 increment.nhn 이라는 요청이 들어오면 컨트롤러에서 호출하는 메서드로 조회수를  
// 증가시킬 글번호를 넘겨받고 mapper를 얻어온 후 MvcBoardDAO 클래스의 조회수를 증가하는
...
// 컨트롤러에 contentView.nhn 이라는 요청이 들어오면 컨트롤러에서 호출하는 메서드로 조회수를  
// 증가시킨 글번호를 넘겨받고 mapper를 얻어온 후 MvcBoardDAO 클래스의 조회수를 증가시킨 글 
// 1건을 얻어오는 select sql 명령을 실행하는 메소드를 실행하고 조회수를 증가 시킨 글 1건을
// request 영역에 저장하는 메소드
	public void selectByIdx(HttpServletRequest request, HttpServletResponse response) {
		...
		// request 객체로 넘어온 조회수를 증가시킨 글번호와 돌아갈 페이지를 받는다.
		int idx = Integer.parseInt(request.getParameter("idx"));
		...
		// 브라우저에 표시할 글 저장된 객체(vo), 작업 후 돌아갈 페이지 번호(currentPage),
		// 줄바꿈에 사용할 이스케이프 시퀀스(\r\n)를 request 영역에 저장한다.
...
// 컨트롤러에 update.nhn이라는 요청이 들어오면 컨트롤러에서 호출하는 메서드로 수정할 정보가
...
// update sql 명령을 실행하는 메소드를 호출하는 메소드	
	public void update(HttpServletRequest request, HttpServletResponse response) {
		...
		// request 객체로 넘어온 수정할 데이터를 받아서 MvcBoardVO 객체에 저장한다.
		MvcBoardVO vo = new MvcBoardVO();
...
// 컨트롤러에 delete.nhn이라는 요청이 들어오면 컨트롤러에서 호출하는 메서드로 삭제할 글번호가
...
// 컨트롤러에 replyInsert.nhn이라는 요청이 들어오면 컨트롤러에서 호출하는 메서드로 답글 정보가
// 저장된 request 객체를 넘겨받고 mapper를 얻어온 후 MvcBoardDAO 클래스의 조건에 만족하는 seq를
// 1씩 증가시키는 update sql 명령을 실행하고 답글을 저장하는 insert sql 명령을 실행하는 
// 메소드를 호출하는 메소드		
	public void replyInsert(HttpServletRequest request, HttpServletResponse response) {
		...
		// 답글 정보를 MvcBoardVO 클래스 객체에 저장한다. 이 때, lev와 seq는 1씩 증가시켜 
		// 저장한다.
		MvcBoardVO vo = new MvcBoardVO();
		vo.setIdx(idx);
		vo.setGup(gup);
		...
		// 같은 글그룹(gup)에서 답글이 표시될 위치(seq) 이후의 모든 글이 출력될 위치를 1씩
		// 증가시키는 메소드를 실행 한다.
		HashMap<String, Integer> hmap = new HashMap<>();
