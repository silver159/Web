...
// 월별 양력과 음력을 크롤링하고 양력, 음력 공휴일을 계산해서 리턴하는 메소드	
	public static ArrayList<LunarDate> solaToLunar(int year, int month) {
		...
		// 1 ~ 12월의 양력과 대응되는 음력을 기억한다.
		ArrayList<LunarDate> lunarList = new ArrayList<>(); 
		String targetSite ="";
		
		// 인수로 넘겨받은 year에 해당되는 1 ~ 12월의 양력에 대응되는 음력을 크롤링해서 
		// 얻어온다.
		for (int i =1; i<=12; i++) {
			targetSite = 
			...
			// 크롤링한 데이터를 기억할 org.jsoup.nodes 패키지의 Document 클래스 객체를 
			// 선언한다.
			Document document = null;
			
			try {
				// org.jsoup 패키지의 Jsoup 클래스의 connect() 메소드로 크롤링할 타겟  
				// 사이트에 접속하고 get() 메소드로 타겟 사이트의 정보를 얻어온다.
				document = Jsoup.connect(targetSite).get();
				// System.out.println(document);
				
				// Document 클래스 객체에 저장된 타겟 사이트의 정보 중에서 날짜(<tr>) 단위로
				// 얻어온다.
				// org.jsoup.select 패키지의 Elements 클래스 객체에 Document 클래스 객체로
				// 읽어드린 내용을 select() 메소드를 사용해서 필요한 정보를 얻어온다.
				Elements elements = document.select("tbody > tr");
				// System.out.println(elements);
				
				// Elements 클래스 객체에는 크롤링된 전체 데이터가 저장되어 있으므로 
				// org.jsoup.nodes 패키지의 Element 클래스 객체를 이용해서 Elements 클래스 
				// 객체에 저장된 데이터를 한 건씩 처리한다.
				for (Element element : elements) {
					// System.out.println(element);
					// System.out.println("=================================================");
					
					// 날짜 단위로(<tr>)로 얻어온 정보에서 양력, 음력 단위(<td>)로 얻어온다.
					...
					// System.out.println(ele.get(3).text()); // 요일
					
					String sola = ele.get(0).text();
					String lunar = ele.get(1).text();
					// System.out.println(String.format("양력 %s은 음력으로 %s입니다.", sola, lunar));
					// 크롤링 끝
		...
		for (int i=0; i<lunarList.size();i++) {
			...
			else if (lunarList.get(i).getMonth() ==4 && lunarList.get(i).getDay() ==5)
			{
				lunarList.get(i).setHoliday("<br><span>식목일</span>");
			}
			...
			// 대체공휴일 
			// => 설날, 삼일절, 석가탄신일, 어린이날, 광복절, 추석, 개천절, 한글날, 
			// 크리스마스가 주말(토, 일)이나 다른 공휴일과 겹치면 그 다음 첫 번째 비공휴일을
			// 대체 공휴일로 한다.
					...
					&& !lunarList.get(i).isLunarFlag())
			{
				...
				if(holiday == 4 || holiday == 5) {
					...
					// 개천절과 추석이 목요일 또는 금요일을 제외한 나머지 요일에 겹쳤을 때
					lunarList.get(i + 2).setHoliday("<br><span>대체공휴일</span>");
				}
			}
			
			// 한글절 대체 공휴일 => 한글절이 주말과 겹쳤을 때
		...
		// 1년에 존재하는 모든 공휴일을 처리했으므로 달력 출력에 사용할 달의 정보만 별도로
		// 리턴시킨다.
		// 인수로 받은 month의 양력에 대응되는 음력을 기억한다.
