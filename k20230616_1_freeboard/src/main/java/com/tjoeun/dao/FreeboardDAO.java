...
// FreeboardService 클래스에서 호출되는 mapper를 넘겨받고 메인글 전체 개수를 얻어오는 
...
// HashMap 객체를 넘겨받고 1페이지 불량의 메인글 목록을 얻어오는 freeboard.xml 파일의 select 
// sql 명령을 실행하는 메소드
...
// FreeboardService 클래스에서 호출되는 mapper와 죄회수를 증가시킬 글번호를 넘겨받고 조회수를
...
// FreeboardService 클래스에서 호출되는 mapper와 죄회수를 증가시킨 글번호를 넘겨받고 조회수를
// 증가시킨 메인글 1건을 얻어오는 freeboard.xml 파일의 select sql 명령을 실행하는 메소드
...
// FreeboardService 클래스에서 호출되는 mapper와 삭제할 메인글의 글번호를 넘겨받고 메인글 1건을
// 삭제하는 freeboard.xml 파일의 delete sql 명령을 실행하는 메소드	
	public void delete(SqlSession mapper, int idx) {
		System.out.println("FreeboardDAO 클래스의 delete()메소드 실행");
		mapper.delete("delete", idx);
	}
	
// FreeboardService 클래스에서 호출되는 mapper와 수정할 정보가 저장된 객체를 넘겨받고 메인글 
// 1건을 수정하는 freeboard.xml 파일의 update sql 명령을 실행하는 메소드		
	public void update(SqlSession mapper, FreeboardVO vo) {
		System.out.println("FreeboardDAO 클래스의 update()메소드 실행");
		mapper.update("update", vo);
	}
	
// reeboardService 클래스에서 호출되는 mapper를 넘겨받고 모든 공지글 목록을 얻어오는 
// freeboard.xml 파일의 select sql 명령을 실행하는 메소드	
