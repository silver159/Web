...
// FreeboardCommentDAO 클래스의 댓글로 저장하는 insert sql 명령을 실행하는 메소드를 호출하는
// 메소드
...
// FreeboardCommentDAO 클래스의 댓글의 갯수를 얻어오는 select sql 명령을 실행하는 메소드를 
// 호출하는 메소드
...
// selectByIdx.jsp 에서 호촐되는 댓글 목록을 얻어올 메인글의 글번호를 넘겨받고 mapper를 얻어온
// 후 FreeboardCommentDAO 클래스의 댓글 목록을 얻어오는 select sql 명령을 실행하는 메소드를
// 호출하는 메소드
...
// FreeboardCommentDAO 클래스의 댓글을 수정하는 update sql 명령을 실행하는 메소드를 호출하는
// 메소드	
...
// FreeboardCommentDAO 클래스의 댓글을 삭제하는 delete sql 명령을 실행하는 메소드를 호출하는
// 메소드	
	public boolean deleteComment(FreeboardCommentVO co) {
		...
		// 댓글을 삭제하기 위해서 입력한 비밀번호와 삭제할 댓글의 비밀번호를 비교하기 위해서
		//삭제할 댓글을 얻어온다.
		FreeboardCommentVO original = dao.selectCommentByIdx(mapper, co.getIdx());
		if(original.getPassword().trim().equals(co.getPassword())) {
