...
// insert.jsp에서 호출되는 테이블에 저장할 메인 카테고리 정보가 저장된 객체를 넘겨받고 mapper를
// 얻어온 후 CategoryDAO 클래스의 메인 카테고리를 테이블에 저장하는 insert sql 명령을 실행하는
// 메소드를 호출하는 메소드
	public void insert(CategoryVO vo) {
		System.out.println("CategoryService 클래스의 insert()메소드 실행");
		// mapper를 얻어온다. SqlSession이 mapper의 정적 클래스
		...
		// 실행한 sql 명령이 테이블을 변경하는 insert, delete, update sql 명령일 경우 작업 
		// 결과를 테이블에 반영시키기 위해서 작업이 완료되면 반드시 commit() 메소드를 실행해야
		// 한다.
		// 테이블이 변경되지 않는 select sql 명령은 commit() 메소드를 실행하지 않아도 상관없다.
		mapper.commit(); // 필수 (select시 선택)
		// mapper를 사용한 데이터베이스 작업이 완료되면 반드시 mapper를 닫아야 한다.
		mapper.close(); // 필수 
	}
	
// list.jsp에서 호출되는 mapper를 얻어온 후 CategoryDAO 클래스의 테이블에 저장된 전체 카테고리 
...
// reple.jsp에서 호출되는 테이블에 저장할 서브 카테고리 정보가 저장된 객체를 넘겨받고 mapper를
// 얻어온 후 CategoryDAO 클래스의 서브 카테고리를 테이블에 저장하는 insert sql 명령을 실행하는
// 메소드를 호출하는 메소드
	public void reply(CategoryVO vo) {
		...
		// 서브 카테고리가 부모 카테고리 바로 아래 나와야 하므로 출력 순서가 부모 카테고리보다
		// 1크다.
		vo.setSeq(vo.getSeq()+1);
		
		// 서브 카테고리를 위치에 맞게 삽입하기 위해서 같은 카테고리 그룹(gup)의 카테고리 출력
		// 순서(seq)를 조정(수정)하는 메소드를 실행한다.
		HashMap<String, Integer> hmap = new HashMap<>();
		hmap.put("gup", vo.getGup());
		hmap.put("seq", vo.getSeq());
		dao.increment(mapper, hmap); // update
		
		// 서브 카테고리가 저장될 위치가 확보되면 서브 카테고리를 테이블에 저장하는 메소드를
		// 호출한다.
...
// delete.jsp에서 호출되는 삭제할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 CategoryDAO
// 클래스의 카테고리 1건을 삭제하는 delete sql 명령을 실행하는 메소드를 호출하는 메소드
...
// delete.jsp 또는 update.jsp에서 호출되는 삭제 또는 수정할 카테고리 번호를 넘겨받고 mapper를
// 얻어온 후 CategoryDAO 클래스의 삭제 또는 수정할 카테고리 1건을 얻어오는 select sql 명령을
// 실행하는 메소드를 호출하는 메소드
...
// delete.jsp에서 호출되는 삭제할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 CategoryDAO 
// 클래스의 삭제할 카테고리 1건을 "삭제된 카테고리 입니다."로 수정하는 update sql 명령을
// 실행하는 메소드를 호출하는 메소드
...
// delete.jsp에서 호출되는 삭제할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 CategoryDAO
// 클래스의 삭제할 카테고리 1건의 deleteCheck 필드의 값을 "YES"로 수정하는 update sql 명령을
// 실행하는 메소드를 호출하는 메소드	
...
// restore.jsp에서 호출되는 복구할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 CategoryDAO
// 클래스의 삭제할 카테고리 1건의 deleteCheck 필드의 값을 "NO"로 수정하는 update sql 명령을
// 실행하는 메소드를 호출하는 메소드	
...
// CategoryDAO 클래스의 카테고리 1건을 수정하는 update sql 명령을 실행하는 메소드를 호출하는 
// 메소드
...
// report.jsp에서 호출되는 신고할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 CategoryDAO 
// 클래스의 카테고리 1건을 deleteReport 필드의 값을 1증가(수정)하는 update sql 명령을 실행하는  
// 메소드를 호출하는 메소드	
...
// CategoryDAO 클래스의 삭제한 카테고리 그룹의 seq를 0부터 1씩 증가하게 다시 부여하는 update 
// sql 명령을 실행하는 메소드를 호출하는 메소드
	public void reSeq (int gup) {
		...
		// ArrayList로 얻어온 카테고리 그룹의 카테고리 개수만큼 반복하며 seq를 0부터 다시 
		// 부여한다.
		for (int i=0; i<gupList.size(); i++) {
			// seq를 수정할 카테고리 일련번호(idx)와 seq를 수정할 데이터(i)를 HasMap 객체에
			// 저장한다.
			HashMap<String, Integer> hmap = new HashMap<>();
