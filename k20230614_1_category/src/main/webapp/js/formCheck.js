...
// 메인 카테고리는 입력하는 form은 1개만 있기 때문에 id를 지정해서 처리한다.
//	$('#form').submit(function (event) { }) 
// => form이라는 id 속성이 지정된 form에서 submit 버튼이 클릭되면 submit() 함수의 인수로 지정한
//    익명 함수가 실행된다. 이 때, 익명 함수의 인수(event)로 실행되는 이벤트가 넘어온다.
	...
	$('#form').submit(function (event) {
		...
		if (category == 0){
			alert('메인 카테고리는 반드시 입력해야 합니다.');
			// 유효성 검사를 실행했는데 규칙에 위배되므로 action 페이지로 넘겨주는 submit 
			// 이벤트의 기본(default) 동작을 중지시킨다.
			// preventDefault(): event로 넘어온 이벤트 실행을 강제로 중지시킨다.
			event.preventDefault();
			
			// category라는 id 속성이 지정된 텍스트 박스의 내용을 지운다.
			$('#category').val('');
			// form이라는 id 속성이 지정된 form에서 reset 이벤트를 실행한다.
			// 반드시 [0]이라고 인덱스를 지정해야 정상적으로 실행된다.
			// $('#form')[0].reset();
			// category라는 id 속성이 지정된 텍스트 박스로 포커스를 옴겨준다.
			$('#category').focus();
		}
	});	
...
// 서브 카테고리를 입력하는 폼의 개수만큼 반복하며 모든 서브 카테고리 폼에 name 속성이 다르게
// 지정한다.

// sub_form이라는 class 속성이 지정된 폼의 개수만큼 each() 함수의 인수로 지정한 익명 함수가
// 반복되서 실행된다.
// each() 함수는 익명 함수로 2개의 인수를 넘기는데 첫 번째 인수는 선택된 객체의 인덱스이고
// 두 번째 인수는 선택된 객체들 중에서 인덱스 번째 객체이다.	
...
// 서브 카테고리를 입력하는 텍스트 박스의 개수만큼 반복하며 모든 서브 카테고리를 입력하는
// 텍스트 박스에 이름이 다른 class를 지정한다.	
