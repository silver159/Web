...
<c:forEach var="vo" items="${categoryList.list}">
	...
	<!-- 서브 카테고리를 입력하는 모든 폼에 다른 name 속성이 지정되야 식별이 가능하므로
	폼 이름을 만든다. multi submit 일때 구별한다. -->
	...
	<form class="sub_form row m-1" action="reply.jsp" method="post" name="${formName}">	
		...
		<div class="col-md-4">
		<!-- idx, gup, lev, seq를 표시하는 텍스트 상자는 텍스트가 완료되면 모두 hidden으로 
		변경된다. -->
			...
			<c:if test="${vo.deleteCheck != 'YES'}"><!-- 삭제된 카테고리가 아닌가?> -->
		...
		<c:if test="${vo.deleteReport <= 10}">
			...
			<div class="col-md-5">
				...
				<c:if test="${vo.deleteCheck == 'YES'}"><!-- 삭제된 카테고리인가> -->
					...
					<%-- <input class="btn btn-outline-success" type="button" value="복구"
						onclick="location.href='restore.jsp'"/> --%>
				</c:if>
				<c:if test="${vo.deleteCheck != 'YES'}"><!-- 삭제된 카테고리가 아닌가?> -->
					...
					<input class="btn btn-success" type="button" value="복구"
						disabled="disabled"/>
				</c:if>
