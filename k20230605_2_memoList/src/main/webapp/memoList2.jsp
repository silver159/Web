...
<% 
// 1. =====================================================================================	
// 페이징 작업에 사용할 8개의 변수를 선언한다. => 계산에 의한 값을 가져야하는 변수는 0으로
// 초기화
	int pageSize = 10; // 1 페이지당 표시하려는 글의 개수
	int totalCount = 0; // 테이블에 저장된 저체 글의 개수
	int totalPage = 0; // 전체 페이지 개수
	int currentPage = 1; // 현재 브라우저에 표시되는 페이지 번호
	int startNo = 0; // 현재 브라우저에 표시되는 글의 시작 인덱스 번호 = mysql은 인덱스가
	// 0부터 시작한다.
	int endNo = 0; // 현재 브라우저에 표시되는 글의 마지막 인덱스 번호, mysql에서는 사용하지
	// 않는다. oracle에서는 사용
...
// 	totalCount 변수에 테이블에 저장된 전체 글의 개수를 얻어와서 저장한다.
	...
	// 테이블에 저장된 글이 있으면 글의 개수를 얻어와서 ResultSet 객체에 저장하고 테이블에
	// 저장된 글이 없으면 0을 얻어와서 ResultSet 객체에 저장하므로 ResultSet 객체에 저장되는
	// 데이터가 무조건 존재하기 때문에 물어보는 동작을 할 필요없다.
	// if (rs.next()) {
		rs.next();
...
// 3. =====================================================================================	
	...
	// 이전 페이지에서 currentPage가 넘어오지 않으면 currentPage는 null이다. null에 parseInt()
	// 메소드를 실행하면 NumberFormatException이 발생되므로 현재 페이지에 표시할 페이지 번호
	// (currentPage)를 받을 때 예외 처리를 해야 한다.
	// currentPage가 정상적으로 넘어오면 넘어온 currentPage를 정수로 바꿔서 currentPage 변수에
	// 저장하고 그렇치 않으면 초기값으로 지정한 1이 유지되게 한다.
	try{
		currentPage = Integer.parseInt(request.getParameter("currentPage"));
		// 현재 화면에 표시할 페이지 번호는 전체 페이지 개수보다 클 수 없으므로 화면에 표시할
		// 페이지 번호가 전체 페이지 개수보다 큰 값이 넘어왔다면 화면에 표시할 페이지 번호를
		// 전체 페이지 개수로 수정한다.
...
// 4. =====================================================================================	
	...
	// mysql은 select sql 명령을 실행했을 때 맨 처음 나오는 글의 인덱스가 0부터 시작되고
	// oracle은 1부터 시작된다.
	startNo = (currentPage - 1) * pageSize;
	// out.println("현재 화면에 표시되는 글 목록의 시작 인덱스 번호: " +startNo+"<br/>");
	// mysql은 limit를 사용하면 되기 때문에 endNo를 계산할 필요가 없지만 oracle을 limit가
	// 없기 때문에 endNo를 계산해야 한다.
	endNo = startNo + pageSize - 1;
	// 마지막 페이지에 표시되는 글의 개수는 반드시 화면에 표시할 글의 개수(pageSize)만큼
	// 표시되지 않는다.
	// 한 페이지에 표시할 마지막 글의 인덱스는 전체 글의 개수보다 커지면 안되므로 마지막 
	// 글의 인덱스가 전체 글의 개수보다 커지면 전체 글의 개수로 endNo를 수정한다.
...
// 5. =====================================================================================	
	...
	// memolist.jsp에서 사용했던 select sql 명령을 수정해서 전체 글을 얻어오던 것을 limit를
	// 사용해서 현재 페이지에 출력할 내용만 얻어오게 수정한다.
