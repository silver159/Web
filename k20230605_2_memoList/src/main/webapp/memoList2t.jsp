...
<form action="memoInsert.jsp" method="post">
	<table width="1000" align="center" border="1" cellpadding="5" cellspacing="0">
		<tr>
			<th colspan="3">아주 아주 많이 허접해 보이는 출첵 게시판 Ver 0.15</th>
		</tr>
		<tr>
			<th width="100">이름</th>
...
//	페이징 작업에 사용할 8개의 변수를 선언한다. => 계산에 의한 값을 가져야하는 변수는 0으로 초기화 한다.
	...
	int endNo = 0; // 현재 브라우저에 표시되는 글으 마지막 인덱스 번호, mysql에서는 사용하지 않는다.
...
//	이전 페이지에서 currentPage가 넘어오지 않으면 currentPage는 null 이다. null에 parseInt() 메소드를
...
<%
	for (int i=1; i<=totalPage; i++) {
		if (currentPage == i) {
			...
			out.println("<button type='button' onclick='location.href=\"?currentPage=" + i + "\"'>" + i +
					"</button>");
