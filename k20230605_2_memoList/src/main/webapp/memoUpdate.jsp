...
// memoList4.jsp에서 넘어오는 수정할 글번호(idx)와 수정 후 돌아갈 페이지 번호(currentPage)를 
// 받는다.
...
// 수정할 글 번호와 수정 후 돌아갈 페이지 번호가 모두 넘어왔나 검사한다.
	if(tempIdx != null && tempIdx.trim().length() != 0 && tempCurrentPage != null &&
		...
		try{
			...
			// 수정할 글번호에 해당되는 글이 테이블에 존재하는가 확인하기 위해서 글번호에 
			// 해당되는 글을 테이블에서 얻어온다.
...
<%			
				...
				// 수정할 글이 테이블에 존재하지 않기 때문에 에러 메시지를 출력하고 
