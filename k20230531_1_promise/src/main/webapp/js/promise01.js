...
const rabbit = {
	...
	color: '회색',
	size: '겁나큼',
	// javascript 객체나 함수는 stringify() 함수로 json 형태로 변환 시킬 수 없다.
	birthDate: new Date(),
	jump: () => console.log('jump')
}
console.log(rabbit);

// json은 key 부분을 반드시 큰 따옴표를 사용하는 문자열로 만들어야 한다.
...
json = JSON.stringify(rabbit, (key, value) => {
	...
	// return value; // 반드시 들어가야 한다. 꼭 적어야 한다. 없으면 이상하게 나온다.
...
// json 객체를 javascript 객체로 변환하기
// parse(json 객체): 인수로 지정된 json 객체를 javascript 객체로 변환한다.

// console.clear(): 콘솔 창에 출력한 내용을 모두 지운다.

// rabbit javascript 객체를 json 객체로 변환한 후 다시 javascript 객체로 변환한다.
json = JSON.stringify(rabbit);
console.log(json);
