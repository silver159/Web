...
// 리턴값이 promise면 비동적으로 실행되는 함수이다.
console.log('fetcjed: ' + fetcjed); // fetcjed: [object Promise] => 전체 문자열
console.log('fetcjed: ', fetcjed); // fetcjed: ▶Promise {<pending>} => 문자열, 객체
...
/*
fetcjed.then(function(response){
	console.log('response: ', response); // response: ▶Response { ~ }
});
fetcjed = fetch('https://jsonplaceholder1.typicode.com/posts'); // 주소 틀림
fetcjed.catch(function(e) {
	console.log('error: ', e); // error:  TypeError: Failed to fetch
});
*/
fetcjed.then(response => console.log('response: ', response)) 
fetcjed = fetch('https://jsonplaceholder1.typicode.com/posts'); 
fetcjed.catch(e =>	console.log('error: ', e));
...
fetch('https://jsonplaceholder.typicode.com/posts')
	.then(function (response){
		...
		*/
		// response.json().then(data => console.log('3. data: ', data))
