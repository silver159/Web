...
function searchCSSSelector() {
	// querySelector(): css 선택자를 이용해서 첫번재 요소 1개를 찾는다. => id 탐색에 주로 사용된다.
	// querySelectorAll(): querySelector() 함수와 똑같이 css 선택자를 이용해서 찾지만 차이점은
	// 모든 요소를 찾는다.
	// querySelector(), querySelectorAll() 함수의 인수로 id를 지정할 때는 '#'으로 시작하고
	// class를 지정할 때는 '.'으로 시작하고 태그는 아무것도 붙이지 않는다.
