...
function joinTest() {
	let fruits = ['apple', 'banana', 'melon', 'margo'];
	let arr = ['111', '222', '333', '444'];
	
	// 배열끼리 덧셈 연산을 실행하면 각각의 배열을 ','로 구분된 문자열을 만들어서 합친다.
	let add = fruits +arr;
	console.log(add); // apple,banana,melon,margo111,222,333,444
	console.log(typeof add); // string
	
	// 2개 이상의 배열을 합치려면 concat() 함수를 사용해야 한다.
	let result = fruits.concat(arr);
	console.log(result) // ['apple', 'banana', 'melon', 'margo', '111', '222', '333', '444']
	console.log(typeof result); // object
	
	// join() 함수는 배열 요소 사이에 인수로 지정한 문자열을 삽입해서 만자열로 만든다,
	let resut2 = fruits.join(',');
	console.log(result2); // apple,banana,melon,margo
	console.log(typeof result2); // string
}

function alphaSort() {
	// sort(): 코드값 순서로 오름차순(사전식) 정렬, 무조건 문자 데이터로 취급해서 정렬한다.
	// 문자화된 숫자(0:48) => 문자('A':65 'a':97) => null
	let arr = ['a', 5, null, 'c', 'b', 'A', 100 ]; // 저렇게 넣으면 null도 문자로 취급한다.
	arr.sort(); // [100, 5, 'A', 'a', 'b', 'c', null];
	console.log(arr);
}
...
function numberSort() {
	...
	// sort() 함수가 자동으로 비교 함수를 데이터를 넘겨주기 때문에 비교 함수를 호출할 때 인수를
	// 적지 않는다.
	arr.sort(comarefn);
	...
	arr.sort((i,j) => j - i); // 내림차순
	console.log(arr); // [10, 9, 8, 7, 4, 3, 1]
}


// numberSort() 함수에서 숫자 데이터 정렬에 사용할 함수를 선언한다. => 정렬 방식을 지정한다.
function comarefn(a,b){
	// 1번째 인수에 저장된 값이 2번째 인수에 저장된 값보다 크면 1(양수), 같으면 0, 작으면
	// -1(음수)를 리턴하게 만든다. => 오름차순 정렬 => 부등호를 반대로 수정하면 내림차순
	// 정렬이 된다.
	/*
	if(a>b){
		return 1;
...
function reverseTest() { // 숫자 쪽은 안됨. 위에 방법으로
...
function arrayIO() {
	...
	// shift(): 배열의 앞 앞의 데이터를 얻어온 후 제거한다.
	let shift = arr.shift();
	console.log(shift);
	console.log(arr);
}
