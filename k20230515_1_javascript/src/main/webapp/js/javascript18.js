...
// => 함수 오버로딩은 지원되지 않는다/
// 함수가 호출될 때 넘어오는 인수를 저장할 변수가 모자라면 모자라는 대로 남으면 남는대로
// 실행한다.
// 자바스크립트의 함수가 호출될 때 실인수는 arguments 객체로 먼저 전달이 되고 arguments 객체에
// 저장된 인수 목록에서 가인수의 개수만큼 반복하며 arguments 객체에 저장된 데이터를 꺼내서
// 가인수에 넣어준다.
...
// closureTest()의 내부 함수임 addValue() 함수가 리턴되므로
...
// 익명 함수 뿐만 아니라 이름이 있는 함수도 변수에 할당할 수 있으므로 goodEdu에 할당된 
// addValue() 함수를 goodEdu(msg) 형태로 실행이 가능하다.
