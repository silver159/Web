...
//		실행할 문장;
//		...;
//		return 값; // 현재 실행의 return 값이 다음 실행의 accumulator에 저장된다.
//	}[, accumulator의 초기치]);
...
//	최초 실행시 accumulator의 초기치를 accumulator에 저장한 후 반복이 시작되고 그 다음부터는 현재
...
function tenOverNumber3(numbers) {
	return numbers.reduce((acc, curr) => {
		if (curr > 10) {
			// 조건을 만족하면 10 이상인 경우이므로 현재 acc 값에 1을 더해서 다음으로 넘겨준다.
			// acc++을 사용하면 현재 acc 값인 0을 다음 실행의 acc에 넣어주기 때문에 최종 결과가
			// 0이 나온다.
			return ++acc; // ++acc와 acc + 1은 같은 결과를 얻을 수 있다.
		} else {
