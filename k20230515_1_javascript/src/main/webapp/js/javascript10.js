...
// 그 다음에는 현재 실행한 함수의 return 값을 다음 실행할 함수의 accumulator에 저장한 후 
// 끝까지 반복한다. 
...
// }[,accumulator의 초기치]) 
// accumulator의 초기치를 생략시 배열의 0번째 index의 값이 초기치가 되고 1번째 index부터
// 반복한다.
// accumulator의 초기치를 지정하면 0번째 index부터 반복한다. <= 대부분
...
// currentValue: 생략 가능, reduce() 함수를 실행하는 배열에 저장된 값이 차례대로 넘어와 
// 저장되는 변수
...
count = numbers2.forEach(function (n) {
	if (n>10){
			count++;
		}
		return count;
...
function tenOverNumber3(numbers){
	return numbers.reduce(function (acc, value){
		if(value > 10){
			// 조건을 만족하면 10보다 큰 경우이므로 현재 acc 값을 1 증가시켜 다음으로 넘겨준다.
			// acc++을 사용하면 현재 acc 값인 0을 다음 실행의 acc에 넣어주기 때문에 최종 결과가
			// 0이 나온다.
			// return acc++; // acc를 리턴시키고 1증가 시킨다.
			return ++acc; // acc를 1증가시켜 리턴 시킨다.
		}else{
...
function alphaCount(alphas) {
	return alphas.reduce(function (acc, value){
		...
		// key가 있으면 key에 할당된 value 값(문자의 개수)을 증가시킨ek.
			acc[value] += 1;
		} else{
			// key가 없으면 1로 초기화시킨다.
			acc[value] = 1;
