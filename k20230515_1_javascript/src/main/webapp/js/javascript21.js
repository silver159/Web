...
function confirmChk() {
	...
	// 데이터가 있으면 데이터가 위치한 index를 리턴하고 데이터가 없으면 -1을 리턴한다.
	if (id.length == 0) {
		div.innerHTML = '<br/><b style="color: red">중복 검사할 아이디를 입력하세요.</b>';
	}else if (ids.indexOf(id)>=0){
		div.innerHTML = '<br/><b><span style="color: red">' + id +
		 '</span>는(은) 사용할 수 없는 아이디입니다.</b>';
	}else{
		div.innerHTML = '<br/><b><span style="color: red">' + id +
		 '</span>는(은) 사용할 수 있는 아이디입니다.</b>&nbsp;&nbsp;'+
		 '<input type="button" value="사용하기" onclick="insertID(\''+id+'\')"/>';
		 // 자바스크립트 함수는 변수에 저장된 문자열을 호출하는 함수의 인수로 전달하려면
		 // 변수명만 사용하면 안된다.
		 // 변수명만 사용하면 변수에 저장된 데이터를 문자열로 인식하는 것이 아니라 변수로
		 // 인식해서 변수가 정의되지 않았다는 에러가 발생된다.
		 // id라는 변수에 'hong'이 저장되어 있을 때 id라는 변수를 함수로 전달하면 'hong'이라는
		 // 문자열로 인식하는 것이 아니고 hong 이라는 변수로 인식한다.
		 // 변수에 저장된 문자열을 함수의 인수로 전달하려면 변수 이름 앞 뒤에 따움표를 붙여서
		 // 전달해야 한다.
		 // 큰 따옴표와 작은 따옴표가 모두 사용된 상태일 경우 또 따옴표가를 찍어야 한다면 \"
		 // \'를 사용해야 한다. 
	}
	
	// 아이디를 입력하지 않았거나 중복되는 아이디일 경우 아이디를 다시 입력하도록 텍스트 박스를
	// 지우고 포커스를 이동시킨다.
