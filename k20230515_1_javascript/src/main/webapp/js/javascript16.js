...
function scopeTest(){
	...
	// 전역 변수와 같은 이름으로 var 를 사용해서 지역변수를 선언하면 더 이상 전역 변수를 
	// 사용할 수 없다.
...
function test1() {
	...
	// alert(window.document.getElementById('var1')); // 'id이름'
	// 자바스크립트의 최상위 객체 window는 생략해도 자동으로 붙는다.
	console.log(document.getElementById('var1'));
	
// innerText: 선택된 객체 내부에 html 태그 구현이 불가능한 문자열을 넣어주거나 얻어온다.
...
function test2() {
	// 지역 변수가 선언되기 전에 사용하면 undefined가 출력된다. => 호이스팅
