...
function testDate1() {
	...
	today.innerHTML = date; // Mon May 22 2023 13:12:39 GMT+0900 (한국 표준시)
	today.innerHTML = date.toDateString(); // Mon May 22 2023
	today.innerHTML = date.toLocaleString(); // 2023. 5. 22. 오후 1:14:11
	today.innerHTML = date.toLocaleDateString(); // 2023. 5. 22.
	today.innerHTML = date.toLocaleTimeString(); // 오후 1:14:53
}
...
function testDate3() {
	...
	// Date 객체를 만들 때 인수로 년, 월, 일, 시, 분, 초를 넣어주면 날짜를 만들수 있다.
	// Date(년, 월, 일)
...
function testDate4() {
	...
	// 얻어온 지정 날짜는 string 타입의 데이터이므로 날짜 연산을 하기 위해서 날짜 데이터로
	// 만든다.
	...
	// getDate(): 날짜를 얻어온다.
	// setDate(): 날짜를 수정한다.
	console.log(date.getDate()+ parseInt(dateinput) - 1); // 52
...
function testDate5() {
	...
	// 수료 날짜에서 지정 날짜를 뺀다.
	let result2 = endDate - startDate; // 15897600000 환산해야 한다.
	// 날짜/시간 데이터를 연산하면 두 시간 데이터 연산 결과를 밀리초 단위로 계산한다.
	console.log(result2);
	console.log(result2/(24*60*60*1000) + 1); // 185
