// 변수(variable)
// 변수가 var, let, const 선언되지 않으면 에러를 발생시키나.
'use strict;';  //ECMA Script5에서 추가

// 변수의 타입
// 기본(원시, promitive) 자료형
//		=> number(숫자), string(문자열), boolean(논리값), null, undefined, symbol(SE6)
...
// var를 사용해서 선언한 변수는 프로그램 어디에서 선언하든지 변수 선언만 항상 맨위로 끌어올려진다.
...
// block scope: 변수가 선언되 {} 블록 내부에서만 사용할 수 있다. => let, const, java 방식
// function scope: 변수가 선언되 함수에서만 사용할 수 있다. => var
// block scope는 변수가 선언되 {}블록 밖으로 나가면 변수가 소멸되고 function scope는 변수가
// 선언ㄷ뇌 함수가 종료되면 변수가 소멸된다. (java는 block spope) (javascript는 function scope)
...
// typeof 명령으로 변수의 자료형을 확인할 수 있따.
console.log('value: '+count+', tyep: '+ typeof count);
...
console.log(`value : ${nagativeInfinity}, type ${typeof nagativeInfinity}`);

// 자바스크립트는 문자열을 나누면 결과가 NaN(Not A Number)이 된다.
