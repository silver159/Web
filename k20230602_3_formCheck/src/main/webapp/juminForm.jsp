...
<!--  
	...
	form check는 submit 버튼이 클릭되면 실행되는 onsubmit 이벤트에서 javascript 함수를 
	실행해서 form에 입력된 데이터가 정상적인 데이터인가 유효성을 검사해서 검사 결과가
	정상이면 true, 오류가 발생되면 false를 리턴하게 만든다. => 기본값은 true이다.
	onsubmit="return javascript함수(this)" 형태로 코딩하며 javascript 함수의 실행 결과
	return 값이 true면 action 속성으로 지정한 페이지로 넘겨주고 리턴값이 false면 현재
	페이지에 그대로 머물러있게 한다.
-->
...
<form action="juminFormOK.jsp" method="post" name="juminForm" onsubmit="return formCheck(this)">
	...
	<input type="text" name="j2" maxlength="7" placeholder="주민등록번호 뒷자리"
		onkeyup="moveNext(this, 7, document.juminForm.sandBtn)"/>
	<input type="submit" name="sandBtn" value="검사하기">
</form>
