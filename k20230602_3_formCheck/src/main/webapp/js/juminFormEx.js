...
// 인수로 넘어온 form의 유효성 검사를 하고 정상이면 true를 리턴하고 오류가 발생되면 오류 메시지를
// 출력하고 false를 리턴하는 함수
function formCheck(obj){
	// obj로 this(form)가 넘어오므로 obj에는 document.juminForm(<form> name)이 저장된다.
	// console.log(obj); 
	// console.log(document.juminForm);
	
	// 주민등록번호 앞 자리에 아무것도 입력하지 않았거나 전부 공백이 입력되었으면 에러 메시지를
	// 출력하고 false를 리턴시킨다.
	...
	// 출력하고 false를 리턴시킨다.
	if(obj.j1.value.trim().lenght != 6){
		alert('주민등록번호 앞 자리 6글자를 입력하세요.')
		obj.j1.value = '';
		obj.j1.focus();
		return false;
	}
	
	// 주민등록번호 앞자리에 숫자만 입력되었나 검사해서 숫자만 입력되지 않았으면 에러 메시지를
	// 출력하고 false를 리턴시킨다.
	// isNaN(): 인수로 지정된 내용이 숫자가 아니면 true, 숫자면 false를 리턴한다.
	// console.log(Number(obj.j1.value)) // NaN => true or false로 변환시켜야 한다.
	if (isNaN(obj.j1.value)){
		alert('주민등록번호 앞 자리를 숫자만 입력하세요.')
		obj.j1.value = '';
		obj.j1.focus();
		return false;
	}
	...
	// 자바스크립트는 문자열끼리 덧셈 연산을 하면 문자열을 연결하지만 뺄셈, 곱셈, 나눗셈은
	// 숫자로 바꿔서 연산한다.
	// 문자열로 구성된 숫자의 덧셈을 하려면 Number() 함수를 이용해 숫자로 변환한 뒤 연산한다.
	
	// 가중치 => 234567892345, 둘리 주민등록번호 => 8304221185600 => 가중치와 곱해서 모두
	// 더하면 143
	...
	// console.log(sum); // 143
	// sum에 저장된 값을 11로 나눈 나머지를 11에서 뺀 결과가 주민등록번호의 마지막 자리와
	// 같아야 한다.
	// 11로 나누 나머지를 11에서 뺀 결과가 10 이상일 경우 10의 자리는 버리고 1의 자리만 
	// 취해서 주민등록번호의 마지막 자리와 비교한다.
