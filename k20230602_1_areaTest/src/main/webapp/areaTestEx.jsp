...
<!--  
	...
	request => 현재 보고 있는 페이지와 현재 보고 있는 페이지의 다음 페이지
...
<% 
	...
	// 영역 변수가 저장되는 기억 공간은 영역 변수 이름을 key로 하고 영역 변수에 저장되는
	// 데이터를 value로 하는 Map<String, Object> 형태의 구조로 value의 자료형이 Object인
	// 이유는 개발자가 어떤 타입의 데이터를 영역 변수에 저장할 지 모르기 때문이다.
	
	pageContext.setAttribute("pageContextVar", "areaTest.jsp의 pageContext 영역 변수");
	
	// 영역이름.getAttribute("영역 변수 이름");와 같이 얻어온다.
	...
	// getAttribute() 메소드로 영역 변수에 저장된 데이터를 얻어오면 Object 타입으로 얻어오기
	// 때문에 영역 변수에 저장된 데이터는 일반 변수에 저장할 때 일반 변수의 데이터 타입으로 
	// 형변환 시켜서 사용해야 한다.
	
	String pageContextVar = (String) pageContext.getAttribute("pageContexVar");
	out.println("pageContext 영역 변수: "+ pageContextVar + "<br/>");
	
	// sendRedirect(): 인수로 지정된 페이지로 넘겨준다.
	// sendRedirect() 메소드는 주소창에 표시되는 페이지 이름이 인수로 지정한 페이지 이름으로
	// 변경된다.
	// 현재 페이지와 sendRedirect() 메소드의 인수로 지정한 페이지는 완전히 분리된다.
	// 새로고침하면 sendRedirect() 메소드의 인수로 지정한 페이지만 다시 실행된다.
	
	// ※ sendRedirect() 메소드는 requeset 영역의 데이터를 가지지 않고 인수로 지정된 페이지로
	// 넘어간다.
	
	// response.sendRedirect("requestTestEx.jsp");
	
	// forward(): 인수로 지정된 페이지로 넘겨준다.
	// forward() 메소드는 주소창에 표시되는 페이지 이름이 forward() 메소드가 실행된 페이지
	// 이름이 그대로 유지된다.
	// 현재 페이지와 forward() 메소드의 인수로 지정한 페이지가 연결된 상태이다.
	// 새로고침하면 주소창에 표시된 forward() 메소드가 실행된 페이지부터 forward() 메소드의
	// 인수로 지정한 페이지까지 다시 실행된다.
	
	// ※ forward() 메소드는 request 영역의 데이터를 가지고 인수로 지정된 페이지로 넘어간다.
	
	pageContext.forward("requestTestEx.jsp");
%>
