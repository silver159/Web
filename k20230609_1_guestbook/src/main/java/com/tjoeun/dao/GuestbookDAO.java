...
public class GuestbookDAO {
	...
	// InsertService 클래스에서 호출되는 mapper와 테이블에 저장할 데이터가 저장된 객체(vo)를
	// 넘겨받고 guestbool.xml 파일의 insert sql 명령을 실행하는 메소드
	public void insert(SqlMapClient mapper, GuestbookVO vo) throws SQLException {
		...
		*/
		// insert("실행할 sql 명령의 id", sql 명령으로 전달한 데이터)
		mapper.insert("insert", vo);
		
	}
	// SelectService 클래스에서 호출되는 mapper를 넘겨받고 guestbool.xml 파일의 테이블에 저장된 
	// 전체 글의 개수를 얻어오는 select sql 명령을 실행하는 메소드
	public int selectCount(SqlMapClient mapper) throws SQLException {
		System.out.println("GuestbookDAO 클래스의 selectCount() 메소드 실행");
		// queryForObject(): select sql 명령의 실행 결과가 1건일 때 사용한다. 
		// => 결과가 Object 클래스 타입으로 리턴된다.
		// queryForList(): select sql 명령의 실행 결과가 여러건일 때일 때 사용한다. 
		// => 결과가 List 인터페이스 타입으로 리턴된다 
		return (int) mapper.queryForObject("selectCount");
	}
	
	// SelectService 클래스에서 호출되는 mapper와 화면에 표시할 페이지의 시작 인덱스 번호와 끝
	// 인덱스 번호가 저장된 HashMap 객체를 넘겨받고 guestbool.xml 파일의 1페이지 분량의 글 
	// 목록을 얻어오는 select sql 명령을 실행하는 메소드
	...
	// SelectService 클래스에서 호출되는 mapper와 수정 또는 삭제할 글번호(idx)를 넘겨받고
	// guestbool.xml 파일의 수정 또는 삭제할 글 1건을 얻어오는 select sql 명령을 실행하는 메소드
	...
	// DeleteService 클래스에서 호출되는 mapper와 삭제할 글번호를 넘겨받고 guestbool.xml 파일의
	...
	// UpdateService 클래스에서 호출되는 mapper와 수정할 정보를 넘겨받고 guestbool.xml 파일의
	...
	// SelectService 클래스에서 호출되는 mapper와 검색어(내용)를 넘겨받고 guestbool.xml 파일의
	// 테이블에 저장된 전체 글 중에서 검색어를 포함하는 글의 개수를 얻어오는 select sql 명령을
	// 얻어오는 메소드
	...
	// SelectService 클래스에서 호출되는 mapper와 화면에 출력할 시작 인덱스와 끝 인덱스, 
	// 검색어(내용)가 저장된 Param 클래스 객체를 넘겨받고 guestbool.xml 파일의 내용에
	// 검색어를 포함하는 1페이지 분량의 글 목록을 얻어오는 select sql 명령을 실행하는 메소드
	...
	// SelectService 클래스에서 호출되는 mapper와 검색어(이름)를 넘겨받고 guestbool.xml 파일의
	// 테이블에 저장된 전체 글 중에서 검색어를 포함하는 글의 개수를 얻어오는 select sql 명령을
	// 얻어오는 메소드
	...
	// SelectService 클래스에서 호출되는 mapper와 화면에 출력할 시작 인덱스와 끝 인덱스, 
	// 검색어(이름)가 저장된 Param 클래스 객체를 넘겨받고 guestbool.xml 파일의 내용에
	// 검색어를 포함하는 1페이지 분량의 글 목록을 얻어오는 select sql 명령을 실행하는 메소드
	...
	// SelectService 클래스에서 호출되는 mapper와 검색어(내용+이름)를 넘겨받고 guestbool.xml 
	// 파일의 테이블에 저장된 전체 글 중에서 검색어를 포함하는 글의 개수를 얻어오는 select sql
	// 명령을 얻어오는 메소드
	...
	// SelectService 클래스에서 호출되는 mapper와 화면에 출력할 시작 인덱스와 끝 인덱스, 
	// 검색어(내용+이름)가 저장된 Param 클래스 객체를 넘겨받고 guestbool.xml 파일의 내용에
	// 검색어를 포함하는 1페이지 분량의 글 목록을 얻어오는 select sql 명령을 실행하는 메소드
	...
	// SelectService 클래스에서 호출되는 mapper와 카테고리 및 검색어가 저장된 Param 클래스 
	// 객체를 넘겨받고 guestbool.xml 파일의 테이블에 저장된 전체 글 중에서 카테고리에 따른
	// 검색어를 포함하는 글의 개수를 얻어오는 select sql 명령을 얻어오는 메소드
	...
	// SelectService 클래스에서 호출되는 mapper와 화면에 출력할 시작 인덱스와 끝 인덱스, 
	// 카테고리, 검색어가 저장된 Param 클래스 객체를 넘겨받고 guestbool.xml 파일의 카테고리별
	// 검색어를 포함하는 1페이지 분량의 글 목록을 얻어오는 select sql 명령을 실행하는 메소드
