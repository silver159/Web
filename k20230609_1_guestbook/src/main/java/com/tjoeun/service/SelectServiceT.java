...
public class SelectServiceT {

	private static SelectServiceT instance = new SelectServiceT();
	private SelectServiceT() { }
	public static SelectServiceT getInstance() {
		return instance;
	}
...
//	GuestbookDAO 클래스의 1페이지 분량의 글 목록을 얻어오는 select sql 명령을 실행하는 메소드를 호출하는 메소드
	public GuestbookList selectList(int currentPage) {
		System.out.println("SelectServiceT 클래스의 selectList() 메소드 실행");
		...
		try {
			...
			// 저장해서 전달하면 되고 데이터 타입이 다르다면 클래스 객체에 저장해서 전달하면 된다.
...
//	글 1건을 얻어오는 메소드를 호출하는 메소드
	public GuestbookVO selectByIdx(int idx) {
		System.out.println("SelectServiceT 클래스의 selectByIdx() 메소드 실행");
		SqlMapClient mapper = MyAppSqlConfig.getSqlMapInstance();
...
//	list.jsp에서 호출되는 브라우저에 표시할 페이지 번호와 검색어(내용)를 넘겨받고 mapper를 얻어온 후 
//	GuestbookDAO 클래스의 1페이지 분량의 검색어를 포함하는 글 목록을 얻어오는 select sql 명령을 실행하는
//	메소드를 호출하는 메소드
	public GuestbookList selectListMemo(int currentPage, String item) {
		System.out.println("SelectServiceT 클래스의 selectListMemo() 메소드 실행");
		SqlMapClient mapper = MyAppSqlConfig.getSqlMapInstance();
		GuestbookList guestbookList = null;
		GuestbookDAO dao = GuestbookDAO.getInstance();
		...
		try {
			...
			// category, item을 같이 넘겨야 하므로 데이터 타입이 다르기 때문에 별도의 클래를 만들고
