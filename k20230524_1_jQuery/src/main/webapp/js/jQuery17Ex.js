...
// <div> 태그의 첫 번째 요소로 <p> 태그를 만들어서 넣어준다.
	$('button:eq(0)').click(()=>{
		// prepend(): 인수로 지정된 요소를 선택된 요소 내부에 첫 번째 요소로 추가한다.
		// $('<tag>'): 인수로 지정된 태그를 만든다.
		// $('div').eq(0).prepend($('<p>')).text('prepend'); // 이렇게 사용하지 않도록 주의한다.
		// $('div').eq(0)에 text가 추가된다.
		
		// prepend() 함수에서 태그를 마들고 필요한 속성까지 모두 지정해야 한다.
		// $('div').eq(0).prepend($('<p>').text('prepend').addClass('prepend'));
		
		// jQuery는 태그를 기억하는 변수를 선언할 때 앞에 '$'를 붙이면 태그를 기억하는 변수가 된다.
...
// <div> 태그의 마지막 요소로 <p> 태그를 만들어서 넣어준다. 
	$('button:eq(1)').click(()=>{
