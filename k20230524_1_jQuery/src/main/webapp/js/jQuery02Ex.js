function a1() {
	...
	$('span').css('color', 'chocolate'); // tag이름: 태그 선택자
}
...
function a4() {
	...
	// javascript
	// let child = document.querySelectorAll('div > ul > li > span');
	// for (let s of child){
	//	  s.style.color = 'lime';
	// }
...
function a6() {
	...
	// $('li:nth-child(n + 3').css('backgroundColor', 'dodgerblue'); // 3번째 모두 선택된다.
	// '*' 연산도 가능한데 '*' 연산자를 사용할 수 없고 n 앞에 숫자가 위치해야 한다.
	$('li:nth-child(2n + 1)').css('backgroundColor', 'orange'); // 홀수 인덱스만 선택한다.
	$('li:nth-child(2n)').css('backgroundColor', 'orange'); // 짝수 인덱스만 선택한다.
	
}

	// first-child: 같은 부모 요소를 가지는 형제 요소 중에서 첫번째 자식 요소만 선택한다.
	// last-child: 같은 부모요소를 가지는 형제 요소 중에서 마지막 자식 요소만 선택한다.
