...
//	clearInterval(): setInterval() 함수의 실행을 중지킨다.

//	setTimeout(): 지정된 시간이되면 함수를 실행한다.
//	setTimeout(실행할 함수, 시간)
//	clearTimeout(): setTimeout() 함수의 실행을 중지킨다.
...
//		클릭한 버튼위의 문자열이 start면 stop으로 변경하고 stop이면 start로 변경한다.
		...
		if ($('button').eq(0).text() == 'start') {
			...
			// console.log('오늘 점심은 ' + $('img').eq(2).attr('alt') + '를(을) 먹어야 합니다.');
			alert('오늘 점심은 ' + $('img').eq(2).attr('alt') + '를(을) 먹어야 합니다.');
		}
		
		// start 버튼이 클릭되면 active라는 클래스를 추가하고 이미지 롤링을 시작한다.
		// stop 버튼이 클릭되면 active라는 클래스를 제가하고 이미지 롤링을 종료한다.
