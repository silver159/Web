...
// 태그이름[속성이름]: 지정된 속서을 가지고 있는 모든 요소를 선택한다.
// 태그이름[속성이름=문자열]: 속성 값이 지정된 문자열과 일치하는 모든 요소를 선택한다. 완전일치
// 태그이름[속성이름~=문자열]: 지정된 문자열이 공백을 경계로 포함된 모든 요소를 선택한다.
// 단어포함. 띄어쓰기로 온전히 떨어져 있어야 한다.
// 태그이름[속성이름*=문자열]: 지정된 문자열이 포함되는 모든 요소를 선택한다.
// 태그이름[속성이름$=문자열]: 지정된 문자열로 끝나는 모든 요소를 선택한다.
// 태그이름[속성이름^=문자열]: 지정된 문자열로 시작하는 모든 요소를 선택한다.
// 태그이름[속성이름!=문자열]: 지정된 문자열고 ㅏ같지 않은 모든 요소를 선택한다.
// 태그이름[속성이름|=문자열]: 지정된 문자열-로 시작하는 모든 요소를 선택한다.
...
function includeSelector() {
	...
	// 태그이름[속성이름~=문자열]: 지정된 문자열이 공백을 경계로 포함된 모든 요소를 선택한다.
	// 단어포함. 띄어쓰기로 온전히 떨어져 있어야 한다.
	$('img[title~=img02]').css('opacity', '0.2'); // img023, img023+ 안됨
	
	// 태그이름[속성이름*=문자열]: 지정된 문자열이 포함되는 모든 요소를 선택한다.
	// 문장포함 
	$('img[title*=02]').css('opacity', '0.2'); // img023, img023+ 됨
	
	// 태그이름[속성이름$=문자열]: 지정된 문자열로 끝나는 모든 요소를 선택한다.
	// $('img[title$=4]').css('opacity', '0.2');
	
	// 태그이름[속성이름^=문자열]: 지정된 문자열로 시작하는 모든 요소를 선택한다.
	// $('img[title^=img]').css('opacity', '0.2');
...
// 만약에 위의 체크 박스가 선택되었을 때 checked 속성의 property는 "true"이다.

// 속성과 프로퍼티 설정
// .attr(): 선택된 요소 집합의 첫 번째 요소로 지정된 속성 값을 리턴하거나 선택된 요소의
//			지정된 속성을 전달받은 값으로 설정한다.
// .removeAttr(): 지정된 속성을 제거한다.
// .prop(): 선택된 요소 집합의 첫 번째 요소로 지정된 프로퍼티 값을 리턴하거나 선택된 요소의
//			지정된 프로퍼티를 전달받은 값으로 설정한다.
