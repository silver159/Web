$(() => { // onload, 대부분 맨 위에 적는다.
	...
	*/
	...
	$('input:text').css('backgroundColor' , 'hotpink'); // input 태그만
	
	// select 태그의 name 속성값이 email인 요소에서 change 이벤트가 발생되면 선택된 option의
	// value 값을 name 속성값이 addr인 텍스트 상자에 넣는다.
	...
	*/
	...
	$('select[name=email]').eq(0).change(() => {
		...
		// $('input[name=addr]:eq(0)').val($('select[name=email]:eq(0) > option:selected').val());
	}); 
	*/
	
	$('select[name=email]:eq(0)').change(function () {
		// 콤보 상자는 1번에 1개만 선택되기 때문에 $('select[name=email]:eq(0) > option:selected').eq(0)와
		// $(this)는 같은 의미로 사용된다.
		// $(this)는 화살표 함수에서 사용할 때 동작 되지 않는다.
		// let emailValue = $('select[name=email]:eq(0) > option:selected').eq(0).val();
	...
	*/
	...
	$('input:checkbox[name=hobbies]').click(() => {
		let msg = [];
		// each(): 선택된 요소들의 인덱스와 요소 자체를 리턴하며 인수로 지정한 함수를 
		// 반복 실행한다.
		// each() 함수는 익명 함수로 인수 2개를 넘긴다. 1번째 인수는 index이고 2번째
		// 인수는 객체이다
		// each() 함수가 익명 함수로 전달하는 인수가 필요 없으면 익명 함수에서 인수를
		// 생략할 수 있다. 
		
		
		$('input:checkbox[name=hobbies]:checked').each((index, obj) => {
