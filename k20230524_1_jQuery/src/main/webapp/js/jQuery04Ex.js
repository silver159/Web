...
// :focus: 현재 포커스를 가지고 있는 요소들을 선택한다.
...
function choice() {
	...
	*/
	...
	// console.log(inputs.value); // undefined. inputs 인덱스를 지정해야 한다.
	console.log(inputs[0].value); // javascript 방식
	console.log(inputs[1].value); 
	console.log('================================'); 
	
	let text = $('input:text').eq(0); // jQuery 방식
	console.log(text.value); // undefined, jQuery 방식
...
function choice3() {
	$('#a').html('<h1>' + $('input:checkbox:eq(0)').val() + '</h1>');
}
...
$(() => {	
	...
	*/
	...
	// jQuery
	// javascript는 onclick, onchange, onload와 같이 이벤트 이름에 'on'으로 시작하지만
	...
	$('select').eq(0).change(() => {	
		...
		// :selected: option 태그 중에서 선택된 모든 요소들을 선택한다.
