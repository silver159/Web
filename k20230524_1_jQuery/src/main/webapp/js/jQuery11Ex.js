// animate() 함수로 여러 css 스타일을 이용해서 사용자 이팩트를 정의할 수 있다.
// $('선택자').animate(
...
// );
// 속도 함수
// swing: 시작, 끝 부분에서 천천히 움직이고 중간부분은 빠르게 움직인다.
...
$(()=>{
	...
	$('p').bind('click', function () {
		...
		// .siblings(): 선택된 요소의 형제 요소 중에서 지정한 선택자에 해당되는 모든
		// 형제 요소를 선택한다.
		...
		*/
		...
		switch (command){
			...
			// fadeTo(재생함수, 투명도): fade 효과에서 사용하는 opacity 속성값을 직접 지정한다.
			// fadeTo() 함수가 실행되서 투명도가 변경되면 변경된 투명도가 유지되며 표시되기
			// 때문에 다시 투명도를 다시 1로 설정하는 처리가 필요할 수 있다.
			case 'fadeTo()':
				$('#img').fadeTo(1000, 0.2);break;
			case 'animate()':
				$('#img').animate(
					// 스타일
					{
