...
// list.jsp에서 1페이지 분량의 글 목록과 페이징 작업에 사용할 변수를 저장해서 request 영역에
// 저장한 GuestbookList 클랙스 객체를 받는다.
	
// request 영역에 저장되는 데이터 타입은 Object 타입이므로 받느시 데이터를 얻어온 다음에 형변환
// 시켜야 한다.
			...
			<table class="table" width="99%" border="1" align="center" cellpadding="5" cellspacing="0">
					<tr>
						<td>
							...
							(<%=vo.getIp()%>)님이
<%							
							// 오늘 입력된 글은 시간만 표시하고 어제 이전에 입력된 글은
							// 날짜만 표시한다.
							/*
...
<% 
	...
	if(guestbookList.getStartPage()>1){
		...
		+(guestbookList.getStartPage()-1)+"'\">이전</button>");
	}else{
	...
	if(guestbookList.getEndPage()<guestbookList.getTotalPage()){
		...
		+(guestbookList.getEndPage()+1)+"'\">다음</button>");
	}else{
	...
	if (guestbookList.getCurrentPage()<guestbookList.getTotalPage()){
		out.println("<button type=\"button\" class='button button1' title=\"마지막 페이지로 이동합니다.\" onclick=\"location.href='?currentPage="
				+guestbookList.getTotalPage()+"'\">마지막</button>");
	}else{
