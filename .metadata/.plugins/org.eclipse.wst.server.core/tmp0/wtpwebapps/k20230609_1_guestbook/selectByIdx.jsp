...
//	out.println(vo);
	...
	if(vo == null){
		// 수정 또는 삭제할 글이 테이블에 존재하지 않으면 에러 메시지를 출력하고 list.jsp로
		// 돌려보낸다.
		out.println("<script>");
		...
		// 수정 또는 삭제할 글이 존재하면 수정 버튼이 클릭된 경우 수정할 글을 브라우저에 
		// 보여주는 페이지(update.jsp)로 삭제 버튼이 클릭된 경우 삭제할 글을 브라우저에
		// 보여주는 페이지(delete.jsp)로 request 영역에 얻어온 글, 돌아갈 페이지 번호,
		// 줄바꿈에 사용할 "\r\n"을 저장해서 넘겨준다.
