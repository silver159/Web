...
<body>
	myInfoOK.jsp입니다. <br>
<!-- jsp를 받는다. String으로 넘어온다. -->	
<%
// form에 입력된 한글 데이터가 post 방식으로 전송될 때 깨지는 현상을 방지한다.
// 한글 깨짐을 방지하려면 최초의 request.getParameter() 메소드가 실행되기 전 (맨처음)
// 아래의 내용을 코딩하면 된다. 무조건 쓰면 된다.
	request.setCharacterEncoding("UTF-8");

// get 방식도 한글이 깨졌었었다. => tomcat server 7.0부터 get 방식은 한글이 깨지지 않는다.
// get 방식에서 한글이 깨지면 서버 환경 설정에서 한글이 깨지지 않도록 설정해야 한다.
// Servers 폴더의 server.xml 파일을 열고 Connector 태그에 URLEncoding="UTF-8" 속성을
// 추가하면 된다. 63줄
// 서버에 추가하기 때문에 프로젝트를 생성할 때 마다 설정할 필요가 없고 이클립스를 다시
// 세팅할 때 하면 된다.

// request.getParameter() 메소드로 이전 페이지에서 submit 버튼이 클릭되면 넘어오는 데이터를 
// 받는다.
// 이전 페이지에서 넘어오는 데이터는 무조건 문자열(String) 형태로 넘어온다.
	...
	out.println(name + "님의 올해 "+age+"살 입니다<br>");
	out.println(name + "님의 올해 "+(age+1)+"살 입니다<br>");
	...
	// checkbox는 여러 항목을 선택할 수 있는데 아래와 같이 request.getParameter()
	// 메소드를 이용해 넘어오는 데이터를 받으면 맨 처음 선택한 값 1개만 받을 수 있다.
	...
	// request.getParameterValues() 메소드로 받아서 배열에 저장해야 한다.
